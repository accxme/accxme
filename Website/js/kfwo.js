...
(async function leaderboard() {
    const response = await fetch('https://api.wls.gg/v5/leaderboards/0-0000-288981132135903232?page=0')
 ...
 var x = setInterval(function() {
   ...
   document.getElementById("timer").innerHTML = "DKW x KFWO 100$ DUOS CUP FINALS " + days + "d " + hours + "h "
   + minutes + "m " + seconds + "s ";
 
   // If the count down is finished, write some text
   if (distance < 0) {
     clearInterval(x);
     document.getElementById("timer").innerHTML = "DKW x KFWO 100$ DUOS CUP FINALS | Scorings are auto updated!";
   }
 }, 1000);